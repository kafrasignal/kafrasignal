...
export default function RegisterPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    const saved = typeof window !== "undefined" ? window.localStorage.getItem("KAFRA-register-theme") : null;
    if (saved === "dark" || saved === "light") setTheme(saved);
  }, []);

  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem("KAFRA-register-theme", theme);
  }, [theme]);

  useEffect(() => {
    const saved = typeof window !== "undefined" ? window.localStorage.getItem("KAFRA-register-design") : null;
    if (saved === "tactical" || saved === "executive") setDesignVariant(saved);
  }, []);

  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem("KAFRA-register-design", designVariant);
  }, [designVariant]);
  ...
  const downloadKeyTxt = () => {
    if (!result) return;
    const lines = [
      "KAFRA SIGNAL SYSTEM - ACCESS KEY",
    ...
    a.href = url;
    a.download = `KAFRA-access-key-${result.access_key}.txt`;
